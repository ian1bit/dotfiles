...
console.log(getStringLength('Ian Pereira'));
...
console.log(stringToLowerCase('CAMPO alegRE de LourDES'))
...
const findCaracterInString = (caractere, string) => {
  return string.indexOf(caractere)
}

console.log(findCaracterInString('i', 'Ian Pereira'));
...
const itemContainsInArray = (item, array) => {
  return array.includes(item)
}

const animals = ['elefante', 'baleia', 'gato', 'cachorro', 'capivara']

console.log(itemContainsInArray('esquilo', animals));
...
const shirtIsBeuatiful = ['Essa', 'camisa', 'bonita']
const buyToMe = ['vou', 'comprar', 'ela', 'para', 'mim.']

const concTwoArrays = (firstArray = [''], secondArray = ['']) => {
  return `${firstArray.join(' ')} ${secondArray.join(' ')}`
}

console.log(concTwoArrays(shirtIsBeuatiful, buyToMe))
...
const series = ['game of thrones', 'breaking bad', 'prison break', 'mr robot']
let seriesWithoutLastItem = []

const removeLastItem = (array) => {
  for (let i = 0; i < array.length -1; i++) {
    seriesWithoutLastItem.push(array[i])
  }
}

removeLastItem(series)

console.log(seriesWithoutLastItem);
