...
const numbersLessThan501 = crazyNumbers.reduce((accumulation, crazyNumber) => {
  if (crazyNumber < 501) {
    accumulation + 1
  }
...
const numbersElevadoAoQuadrado = numbers.map(number => number**2)

console.log(numbersElevadoAoQuadrado)
...
const nameCartGames = cart.reduce((acc, game) => {
  return `${acc}- ${game.name} \n`
}, '')

console.log(nameCartGames)
...
const moviesBefore2000Year = tarantinoMovies.filter(movie => movie.release < 2000)

console.log(moviesBefore2000Year)
